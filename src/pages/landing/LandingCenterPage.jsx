...
import ExploreBlogs from "../center/ExploreBlogs";
...
const branchAddresses = {
    "chennai-anna-nagar": {
        ...
        metro: "Thirumangalam Metro Station (5 mins walk away)",
        hours: "Open 24/7"
    },
    "chennai-omr": {
...
function LandingCenterPage() {
    ...
    if (branchParam) {
        for (const [cKey, cData] of Object.entries(centersData)) {
            if (cData.branches[branchParam]) {
                city = cKey;
                branch = branchParam;
                cityData = cData;
                branchData = cData.branches[branchParam];
                break;
            }
            if (branchParam.startsWith(`${cKey}-`)) {
                const slug = branchParam.slice(cKey.length + 1);
                if (cData.branches[slug]) {
                    city = cKey;
                    branch = slug;
                    cityData = cData;
                    branchData = cData.branches[slug];
                    break;
                }
            }
        }
    ...
    const branchId = city && branch ? `${city}-${branch}` : null;
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="hidden lg:block lg:col-span-1">
                        {/* <ExploreBlogs /> */}
                        <Products city={cityData?.name} theme={theme} />
                    </div>
                </div>
            </div>
...
export default LandingCenterPage;
