...
const EditProduct = ({ onClose, tags, categories, onSuccess, productId }) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch(`${API_URL}/api/admin/products/get/${productId}`, {
                    method: "GET",
                    headers: {
                ...
                const selectedCategory = categoryOptions.find(
                    (c) => String(c.value) === String(product.category_id)
                ) || null;
                ...
                // Parse available booking days
                let availableBookingDays = {
                    sun: true, mon: true, tue: true, wed: true, thu: true, fri: true, sat: true
                };
                if (product.available_booking_days) {
                    try {
                        const enabledDays = JSON.parse(product.available_booking_days);
                        availableBookingDays = {
                            sun: enabledDays.includes('sun'),
                            mon: enabledDays.includes('mon'),
                            tue: enabledDays.includes('tue'),
                            wed: enabledDays.includes('wed'),
                            thu: enabledDays.includes('thu'),
                            fri: enabledDays.includes('fri'),
                            sat: enabledDays.includes('sat'),
                        };
                    } catch (e) {
                        console.warn('Failed to parse available_booking_days:', e);
                    }
                }

                // Parse discounts - they come as an array from the API
                let discounts = [];
                if (product.discounts && Array.isArray(product.discounts)) {
                    discounts = product.discounts.map(discount => ({
                        type: discount.type,
                        from_value: discount.from_value,
                        to_value: discount.to_value,
                        discount_cost: discount.discount_cost
                    }));
                }
                ...
                setFormData({
                    ...
                    tags: selectedTags,
                    booking_period: product.booking_period || "fixed_blocks",
                    booking_length: Number(product.booking_length) || 1,
                    calendar_type: product.calendar_type || "hour",
                    multi_day_selection: Boolean(product.multi_day_selection),
                    max_bookings_per_block:
                        product.max_bookings_per_block && Number(product.max_bookings_per_block) > 0
                            ? String(product.max_bookings_per_block)
                            : "",

                    show_remaining_bookings: Boolean(product.show_remaining_bookings),
                    cost: product.cost ? String(product.cost) : "",
                    cost_suffix: product.cost_suffix || "",
                    status: product.status || "active",
                    available_booking_days: availableBookingDays,
                    discounts: discounts,
                });

                // Set initial thumbnail URL for display
                if (product.thumbnail) {
                    setInitialThumbUrl(`${API_URL}/${product.thumbnail}`);
                } else {
                    setInitialThumbUrl(null);
                }
                setThumbnailPreview(null);
    ...
    const validateThumbnail = (thumbnail) => {
        // For editing, thumbnail is not required if there's an existing one
        if (!thumbnail && !initialThumbUrl) {
            return "Thumbnail is required";
        }
        if (thumbnail && thumbnail.size > 5 * 1024 * 1024) { // 5MB limit
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (removeThumb) payload.append("remove_thumbnail", "1");
            ...
            const res = await fetch(`${API_URL}/api/admin/products/update/${productId}`, {
                method: "POST",
                headers: {
            ...
            >
                ...
                <div className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-600 mt-1">Update your product information</p>
                    </div>
                    <button
                        onClick={onClose}
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                        ...
                                        <div className="lg:col-span-2">
                                            ...
                                            <input
                                                ...
                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors.title
                                        ...
                                        <div className="flex items-end space-x-3 ">
                                            ...
                                            <div className="w-1/3">
                                                ...
                                                <input
                                                    ...
                                                    className={`w-full border rounded-md px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:border-transparent ${errors.booking_length
                                                        ? 'border-red-500 focus:ring-red-500'
                                                        : 'border-gray-300 focus:ring-blue-500'
                                                        }`}
                                                />
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors.max_bookings_per_block
                                                    ? 'border-red-500 focus:ring-red-500'
                                                    : 'border-gray-300 focus:ring-blue-500'
                                                    }`}
                                            />
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                onWheel={(e) => e.target.blur()}
                                                placeholder="0"
                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors.cost
                                                    ? 'border-red-500 focus:ring-red-500'
                                                    : 'border-gray-300 focus:ring-blue-500'
                                                    }`}
                                            />
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors.cost_suffix
                                                    ? 'border-red-500 focus:ring-red-500'
                                                    : 'border-gray-300 focus:ring-blue-500'
                                                    }`}
                                            />
                                                ...
                                                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <select
                                                            ...
                                                            className={`w-full border rounded-md px-3 py-3 text-gray-900 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_type`]
                                                                ? 'border-red-500 focus:ring-red-500'
                                                                : 'border-gray-300 focus:ring-blue-500'
                                                                }`}
                                                        >
                                                    ...
                                                    <div>
                                                        ...
                                                        {discount.type === "count" && (
                                                            <input
                                                                ...
                                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_from_value`]
                                                                    ? 'border-red-500 focus:ring-red-500'
                                                                    : 'border-gray-300 focus:ring-blue-500'
                                                                    }`}
                                                            />
                                                        ...
                                                        {discount.type === "date_range" && (
                                                            <input
                                                                ...
                                                                value={discount.from_value}
                                                                onChange={(e) => handleDiscountChange(index, 'from_value', e.target.value)}
                                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_from_value`]
                                                                    ? 'border-red-500 focus:ring-red-500'
                                                                    : 'border-gray-300 focus:ring-blue-500'
                                                                    }`}
                                                            />
                                                        ...
                                                        {discount.type === "exact_day" && (
                                                            <select
                                                                value={discount.from_value}
                                                                onChange={(e) => handleDiscountChange(index, 'from_value', e.target.value)}
                                                                className={`w-full border rounded-md px-3 py-3 text-gray-900 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_from_value`]
                                                                    ? 'border-red-500 focus:ring-red-500'
                                                                    : 'border-gray-300 focus:ring-blue-500'
                                                                    }`}
                                                            >
                                                    ...
                                                    <div>
                                                        ...
                                                        {discount.type === "count" && (
                                                            <input
                                                                ...
                                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_to_value`]
                                                                    ? 'border-red-500 focus:ring-red-500'
                                                                    : 'border-gray-300 focus:ring-blue-500'
                                                                    }`}
                                                            />
                                                        ...
                                                        {discount.type === "date_range" && (
                                                            <input
                                                                ...
                                                                value={discount.to_value}
                                                                onChange={(e) => handleDiscountChange(index, 'to_value', e.target.value)}
                                                                className={`w-full border rounded-md px-3 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_to_value`]
                                                                    ? 'border-red-500 focus:ring-red-500'
                                                                    : 'border-gray-300 focus:ring-blue-500'
                                                                    }`}
                                                            />
                                                        ...
                                                        {discount.type === "exact_day" && (
                                                            <select
                                                                value={discount.to_value}
                                                                onChange={(e) => handleDiscountChange(index, 'to_value', e.target.value)}
                                                                className={`w-full border rounded-md px-3 py-3 text-gray-900 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_to_value`]
                                                                    ? 'border-red-500 focus:ring-red-500'
                                                                    : 'border-gray-300 focus:ring-blue-500'
                                                                    }`}
                                                            >
                                                    ...
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            className={`w-full border rounded-md px-3 py-2 text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:border-transparent ${errors[`discount_${index}_discount_cost`]
                                                                ? 'border-red-500 focus:ring-red-500'
                                                                : 'border-gray-300 focus:ring-blue-500'
                                                                }`}
                                                        />
